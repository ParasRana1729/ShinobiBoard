    ...
    <main className="space-y-8 py-8 px-4 sm:px-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-white/[0.08] pb-6">
        <div className="space-y-2">
          ...
          <div className="h-4 w-96 max-w-full rounded bg-white/[0.05] animate-pulse" />
        </div>

        {/* Search input skeleton */}
        <div className="h-10 w-full md:w-72 rounded-xl bg-white/[0.06] animate-pulse" />
      </div>
      ...
      <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
        {Array.from({ length: 6 }).map((_, i) => (
          <div
            key={i}
            className="rounded-3xl border border-white/[0.08] bg-surface-card p-6 shadow-xl space-y-4"
          >
            ...
            <div className="space-y-2">
              <div className="h-6 w-3/4 rounded-lg bg-white/[0.1] animate-pulse" />
              <div className="h-3.5 w-1/2 rounded bg-white/[0.05] animate-pulse" />
            </div>
            ...
            <div className="space-y-1.5 pt-2">
              ...
              <div className="h-2 w-full rounded-full bg-white/[0.06] overflow-hidden">
                <div className="h-full w-2/3 bg-white/[0.1] rounded-full animate-pulse" />
              </div>
            </div>

            {/* Action button skeleton */}
            <div className="pt-2">
              <div className="h-9 w-full rounded-xl bg-white/[0.08] animate-pulse" />
            </div>
          </div>
        ))}
      </div>
