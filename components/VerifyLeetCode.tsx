...
export function VerifyLeetCode({ current }: { current: string | null }) {
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl border border-white/[0.08] bg-surface/90 p-5 sm:p-6 shadow-xl backdrop-blur-md">
      ...
      <div className="flex items-center justify-between">
        ...
        {current && (
          <div className="flex items-center gap-1.5 rounded-full border border-emerald-500/30 bg-emerald-950/40 px-2.5 py-0.5 text-xs font-semibold text-emerald-400">
      ...
      {current ? (
        <div className="mt-4 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-white/[0.06] bg-ink/60 p-3.5">
          <div className="flex items-center gap-2">
            <span className="font-mono text-sm font-bold text-amber-300">@{current}</span>
            <a
              href={`https://leetcode.com/${current}`}
              target="_blank"
              rel="noreferrer"
              className="inline-flex items-center gap-1 rounded-md bg-white/[0.05] px-2 py-1 text-[11px] font-medium text-slate-300 hover:bg-white/[0.1] hover:text-white transition-colors"
            >
              <span>Profile</span>
          ...
          <button
            onClick={unlink}
            disabled={busy}
            className="flex items-center gap-1.5 rounded-lg border border-rose-500/30 bg-rose-950/30 px-3 py-1.5 text-xs font-semibold text-rose-300 hover:bg-rose-900/40 hover:border-rose-500/50 transition-all disabled:opacity-50"
          >
        ...
        <div className="mt-4">
          <div className="flex flex-col gap-2.5 sm:flex-row">
            <div className="relative flex-1">
              <span className="absolute left-3.5 top-1/2 -translate-y-1/2 font-mono text-sm text-slate-500">@</span>
              <input
                value={username}
                onChange={(e) => setUsername(e.target.value)}
                placeholder="leetcode_username"
                className="w-full rounded-xl border border-white/[0.1] bg-ink/80 py-2.5 pl-8 pr-3.5 font-mono text-sm text-white placeholder-slate-500 shadow-inner focus:border-amber-400 focus:outline-none focus:ring-1 focus:ring-amber-400 transition-all"
              />
            </div>
            <button
              onClick={link}
              disabled={busy || !username.trim()}
              className="flex items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-amber-400 to-amber-500 px-5 py-2.5 text-xs font-bold text-black shadow-glow hover:brightness-110 active:scale-[0.98] transition-all disabled:opacity-50 disabled:pointer-events-none"
            >
